...
def _action_hora2ros2_left(actions):
    ...
    Direct mapping:
      ...
      ros2[4-7]   = hora[12-15] (index)
      ros2[8-11]  = hora[8-11]  (middle)
      ros2[12-15] = hora[0-3]   (ring)
    """
...
def _obs_ros22hora_left(o):
    ...
    Direct mapping:
      hora[0-3]   = ros2[12-15] (ring)
      hora[4-7]   = ros2[0-3]   (thumb)
      hora[8-11]  = ros2[8-11]  (middle)
      hora[12-15] = ros2[4-7]   (index)
    """
...
class LeftHardwarePlayer:
    ...
    def _control_step(self):
        ...
        ros2_cmd = _action_hora2ros2_left(cmd)
        ...
        if q_pos is not None:
            hora_q = _obs_ros22hora_left(q_pos)
    ...
    def deploy(self):
        ...
        self.allegro = start_allegro_io(side='left', use_side_prefix=self.use_side_prefix)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Left Hand Allegro Deployment")
    ...
    parser.add_argument("--use-side-prefix", action="store_true",
                        help="Use side-specific joint names (ahl_*) for two-hands setup")
    ...
    agent = LeftHardwarePlayer(hz=args.hz, device=device)
